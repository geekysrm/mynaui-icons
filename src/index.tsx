...
function buildSVG(svg: string, type: string): string {
  const commonProps = `xmlns="http://www.w3.org/2000/svg" width="${scale * 24}" height="${scale * 24}" viewBox="0 0 24 24"`;
  return type === "regular"
    ? `<svg ${commonProps} fill="none" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" stroke="${color}">${svg}</svg>`
    : `<svg ${commonProps} fill="${color}">${svg}</svg>`;
}
